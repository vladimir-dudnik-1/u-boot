    ...
    for(uhs_latency = latency_wr_2kM[1]; uhs_latency > 0; uhs_latency --)
    {
        ...
        for(i = 15; i >= 0; i --)
        {
            ...
            if (cal_done_flag == 1)
                return 0;
            tmpVal = BL_RD_REG(PSRAM_UHS_BASE,PSRAM_UHS_UHS_CMD)>>24;
            uhs_phy_printf_debug("PSRAM_UHS_UHS_CMD>>24 1st dqs--, 0x%x\r\n",tmpVal);
            if(tmpVal == ((2<<3)+uhs_latency_code) && (tmpVal_1 == 1)){
                uhs_phy_printf_debug("reg read pass by dqs= %d\r\n",i); 
                // reg_dqs = i;
                dqs_dq_flag = 1;
                break;
            }              
            else{
                uhs_phy_printf_debug("reg read fail by dqs= %d\r\n",i);
            }
			// if(tmpVal_1 == 0){
        ...
        for(i = 15; i >=0; i --)
        {
            ...
            if (cal_done_flag == 1)
                return 0;
            tmpVal = BL_RD_REG(PSRAM_UHS_BASE,PSRAM_UHS_UHS_CMD)>>24;
            uhs_phy_printf_debug("PSRAM_UHS_UHS_CMD>>24 1st dq--, 0x%x\r\n",tmpVal);
            if(tmpVal == ((2<<3)+uhs_latency_code) && (tmpVal_1 == 1)){
                uhs_phy_printf_debug("reg read pass by dq= %d\r\n",i);
                // reg_dq = i;
                dqs_dq_flag = 1;
                break; 
            }  
            else{
                uhs_phy_printf_debug("reg read fail by dq= %d\r\n",i);
            }
			// if(tmpVal_1 == 0){
			// 	uhs_phy_printf_debug("read done not found!!!\r\n");
			// 	// uhs_reset(datarate);
        ...
        if (dqs_dq_flag == 1){
            ...
            uhs_phy_printf_debug("reg read cal 2nd by latency= %d\r\n",uhs_latency);
            ...
            for(i = 15; i >= 0; i --)
            {
                ...
                tmpVal = BL_RD_REG(PSRAM_UHS_BASE,PSRAM_UHS_UHS_CMD)>>24;
                uhs_phy_printf_debug("PSRAM_UHS_UHS_CMD>>24 2nd dqs--, 0x%x\r\n",tmpVal);
                if(tmpVal == ((2<<3)+uhs_latency_code))
                {
                    uhs_phy_printf("reg read pass by --dqs= %d\r\n",i);
                    reg_dqs += i;
                    dqs_dq_flag = 1;
                    break;
                }
            ...
            for(i = 15; i >= 0; i --)
            {
                ...
                CHECK_ERR_FLAG(uhs_reg_r,(0,0));
                tmpVal = BL_RD_REG(PSRAM_UHS_BASE,PSRAM_UHS_UHS_CMD)>>24;
                uhs_phy_printf_debug("PSRAM_UHS_UHS_CMD>>24 2nd dq--, 0x%x\r\n",tmpVal);
                if(tmpVal == ((2<<3)+uhs_latency_code))
                {
                    uhs_phy_printf("reg read pass by --dq= %d\r\n",i);
                    reg_dq += i;
                    dqs_dq_flag = 1;
                    break;
                }
            ...
            uhs_phy_printf("reg_read_cal pass, latency= %d, dqs= %d, dq= %d\r\n",uhs_latency,reg_dqs,reg_dq);
            break;
        }
    }
...
uint8_t reg_write_cal(void)  //only need sweep dqs
{
    ...
    for(i = 15; i >=0; i --)
    {
        ...
        if(tmpVal == (16 + 32))
            {
                uhs_phy_printf("reg write pass by -- dqs1= %d\r\n",i); 
                reg_dqs1 = i;
                dqs_dq_flag1 = 1;
                break; 
            }              
        else{
            uhs_phy_printf_debug("reg write fail by -- dqs1= %d\r\n",i);
        }
    }        
    // sweep2 dqs 
    for(i = 0; i <=15; i ++)
    {
        ...
        if(tmpVal == (8 + 16))
            {
                uhs_phy_printf("reg write pass by ++ dqs2= %d\r\n",i);
                reg_dqs2 = i;
                dqs_dq_flag2 = 1;
                break; 
            }              
        else{
            uhs_phy_printf_debug("reg write fail by ++ dqs2= %d\r\n",i);
        }
    }           
    ...
    if(tmpVal == 0){   
        ...
        uhs_phy_printf("reg write cal pass dqs= %d, dq= %d\r\n",reg_dqs,reg_dq);
    }              
    else{
        reg_write_err = 2;
        return uhs_err_handler(UHS_REG_WRITE_CAL_ERR);
    ...
    uhs_phy_printf("reg_write_cal return dqs_dq_delta= %d\r\n",dqs_dq_delta);
...
uint8_t array_read_fix(uint32_t addr,uint32_t len,uint32_t data0,uint32_t data1)
{
    ...
    for(i = 0; i < (len>>2); i ++)
    {
        ...
        if (print_flag && i == 0){
            uhs_phy_printf("addr 0x%x, write_val 0x%x, read_val 0x%x\r\n",addr_tmp,data,data_read);
        }
        if(data_read != data){
            array_read_pass = 0;
            uhs_phy_printf("addr 0x%x, write_val 0x%x, read_val error 0x%x\r\n",addr_tmp,data,data_read);
            break;
        }
    }
    if (print_flag){
...
uint8_t array_read_latency_cal(void)
{
    ...
    for(uhs_latency = latency_wr[1]; uhs_latency > 0; uhs_latency --)
    {
        ...
        uhs_phy_printf_debug("array read cal by latency= %d\r\n",uhs_latency);
        set_uhs_latency_r(uhs_latency);
        ...
        for (ck_i = 0; ck_i < (sizeof(array_ck_dly_drv_val)/sizeof(array_ck_dly_drv_val[0])); ck_i++){
            ...
            uhs_phy_printf_debug("array read cal by ck= %d\r\n",array_ck_dly_drv);
            ...
            for(i = 15; i >= 0; i --)
            {
                ...
                if(dqs_flag == 1)
                {
                    uhs_phy_printf("array_read_dqs_dq_cal pass by --dqs= %d\r\n",i); 
                    array_dqs += i;
                    rwindow += i;
            ...
            for(i = 15; i >= 0; i --)
            {
                ...
                if(dq_flag == 1)
                {
                    uhs_phy_printf("array_read_dqs_dq_cal pass by --dq= %d\r\n",i);  
                    array_dq += i;
                    rwindow += i;
                    break; 
                }                  
            ...
            if(dqs_flag == 1 || dq_flag == 1){
                ...
                uhs_phy_printf("array_read_dqs_dq_cal pass by array_dqs= %d, array_dq= %d\r\n",array_dqs,array_dq);
                break;
            }
        }
        if(dqs_flag == 1 || dq_flag == 1){
            uhs_phy_cal_res->rl = uhs_latency;
            uhs_phy_printf("array_read_latency_cal pass, latency= %d, ck= %d\r\n",uhs_latency,array_ck_dly_drv);
            break;
        }
    }
...
uint8_t array_write_ck_cal(void)
{
    ...
    for(array_ck_dly_drv = 4; array_ck_dly_drv <= 15; array_ck_dly_drv ++)
    {
        ...
        if (flag_2 == 0){
            uhs_phy_printf("array_write_ck_cal ck++ = %d, flag_2 = %d\r\n",array_ck_dly_drv,flag_2);
        }
        if(flag_2 == 0 && flag_ck2 == 0){
            flag_ck2 = 1;
            uhs_phy_printf("array_write_ck_cal fail by ++ck= %d\r\n",array_ck_dly_drv);
    ...
    for(array_ck_dly_drv = 15; array_ck_dly_drv >= 4; array_ck_dly_drv --)
    {
        ...
        if (flag_1 == 0){
            uhs_phy_printf("array_write_ck_cal ck-- = %d, flag_1 = %d\r\n",array_ck_dly_drv,flag_1);
        }
    ...
    uhs_phy_printf("array_write_ck_cal pass, ck= %d\r\n",array_ck_dly_drv);
...
uint8_t array_write_dqs_dq_cal(void)
{
    ...
    {  
        ...
        if(dqs_flag1 == 1)
        {
            uhs_phy_printf("array_write_dqs_dq_cal pass by -- dqs1= %d\r\n",i);
        ...
        {
            uhs_phy_printf_debug("array_write_dqs_dq_cal fail by -- dqs1= %d\r\n",i);
        }
    }
    for(i = 0; i <= 15; i ++)
    {
        ...
        if(dqs_flag2 == 1)
        {
            uhs_phy_printf("array_write_dqs_dq_cal pass by ++ dqs2= %d\r\n",i);
        ...
        {
            uhs_phy_printf_debug("array_write_dqs_dq_cal fail by ++ dqs2= %d\r\n",i);
        }
    }
    ...
    uhs_phy_printf("array_write_dqs_dq_cal pass by array_dqs_dly_drv= %d,array_dq_dly_drv= %d\r\n",array_dqs_dly_drv,tmpVal);
    // set dqs by register write cal result 
            ...
            for (wdq_i = 0; wdq_i < (sizeof(wdq_val)/sizeof(wdq_val[0])); wdq_i++){
                ...
                for (wdqs = 0; wdqs <= 15; wdqs++){
                    ...
                    for (rdqs = 0; rdqs <= 15; rdqs++){
                        ...
                        CHECK_ERR_FLAG(uhs_reg_r,(0,1));
                        tmpVal = BL_RD_REG(PSRAM_UHS_BASE,PSRAM_UHS_UHS_CMD)>>24;
                        uhs_phy_printf_debug("PSRAM_UHS_UHS_CMD>>24 = 0x%x, rdqs++\r\n",tmpVal);
                        if(tmpVal == ((2<<3)+uhs_latency_code)){
                            uhs_phy_printf("LATENCY_CODE_WRITE_SUCCESS, wl = %d, wdqs = %d, wdq = %d, rl = %d, rdqs = %d, rdq = %d\r\n",wl,wdqs,wdq,rl,rdqs,rdq);
                            return 0;
                        }
                        else{
                            uhs_phy_printf_debug("LATENCY_CODE_WRITE_FAIL, wl = %d, wdqs = %d, wdq = %d, rl = %d, rdqs = %d, rdq = %d\r\n",wl,wdqs,wdq,rl,rdqs,rdq);
                        }
                    }
                    rdqs = 0;
                    cfg_dqs_rx(rdqs);
                    for (rdq = 0; rdq <= 15; rdq++){
                        ...
                        CHECK_ERR_FLAG(uhs_reg_r,(0,1));
                        tmpVal = BL_RD_REG(PSRAM_UHS_BASE,PSRAM_UHS_UHS_CMD)>>24;
                        uhs_phy_printf_debug("PSRAM_UHS_UHS_CMD>>24 = 0x%x, rdq++\r\n",tmpVal);
                        if(tmpVal == ((2<<3)+uhs_latency_code)){
                            uhs_phy_printf("LATENCY_CODE_WRITE_SUCCESS, wl = %d, wdqs = %d, wdq = %d, rl = %d, rdqs = %d, rdq = %d\r\n",wl,wdqs,wdq,rl,rdqs,rdq);
                            return 0;
                        }
            ...
            for (wdqs = 0; wdqs <= 15; wdqs++){
                ...
                for (rl = latency_wr[1]; rl >= 35; rl--){
                    ...
                    for (rdqs = 0; rdqs <= 15; rdqs++){
                        ...
                        if(flag == 1){
                            uhs_phy_printf("INIT_ARRAY_WRITE_SUCCESS, rdqs++, wl = %d, wdqs = %d, wdq = %d, rl = %d, rdqs = %d, rdq = %d\r\n",wl,wdqs,wdq,rl,rdqs,rdq);
                            print_flag = 0;
                            #if CAL_MODE == 2
                            if(wl != 13){
                            ...
                            uhs_phy_printf_debug("INIT_ARRAY_WRITE_FAIL, rdqs++, wl = %d, wdqs = %d, wdq = %d, rl = %d, rdqs = %d, rdq = %d\r\n",wl,wdqs,wdq,rl,rdqs,rdq);
                        }                    
                    }
                    rdqs = 0;
                    cfg_dqs_rx(rdqs);
                    for (rdq = 0; rdq <= 15; rdq++){
                        ...
                        flag = array_read_fix(addr_rarray,len,data0_rarray,data1_rarray);
                        if(flag == 1){
                            uhs_phy_printf("INIT_ARRAY_WRITE_SUCCESS, rdq++, wl = %d, wdqs = %d, wdq = %d, rl = %d, rdqs = %d, rdq = %d\r\n",wl,wdqs,wdq,rl,rdqs,rdq);
                            print_flag = 0;
                            #if CAL_MODE == 2
                            if(wl != 13){
                                init_array_write_err = 2;
                            ...
                            uhs_phy_printf_debug("INIT_ARRAY_WRITE_FAIL, rdq++, wl = %d, wdqs = %d, wdq = %d, rl = %d, rdqs = %d, rdq = %d\r\n",wl,wdqs,wdq,rl,rdqs,rdq);
                        }                    
                    }
                }
            }
    ...
    for (wl_i = 0; wl_i < (sizeof(wl_val)/sizeof(wl_val[0])); wl_i++){
        ...
        for (wdq_i = 0; wdq_i < (sizeof(wdq_val)/sizeof(wdq_val[0])); wdq_i++){
            ...
            for (wdqs = 0; wdqs <= 15; wdqs++){
                ...
                CHECK_ERR_FLAG(uhs_reg_r,(2,0));
                tmpVal = BL_RD_REG(PSRAM_UHS_BASE,PSRAM_UHS_UHS_CMD)>>24;
                uhs_phy_printf_debug("MA2 = 0x%x\r\n",tmpVal);
                if(tmpVal == 3){
                    uhs_phy_printf("2kM_REG_WRITE_SUCCESS, wl = %d, wdqs = %d, wdq = %d\r\n",wl,wdqs,wdq);
                    return 0;
                }
                else{
                    uhs_phy_printf_debug("2kM_REG_WRITE_FAIL, wl = %d, wdqs = %d, wdq = %d\r\n",wl,wdqs,wdq);
                }
            }
        }
    }
...
uint8_t self_cal(void)
{
    ...
    // uhs_phy_printf("START_CAL_AT %dMbps\r\n",datarate);
...
uint8_t uhs_diagonal_test(uint32_t data0,uint32_t data1)
{
    ...
    for (RA = 0x0; RA <= RowAddr; RA++){
        ...
        for (CA = CA_init; CA <= CA_init + 0x7; CA = CA + 2){
            ...
            if(dataTmp != data0){
                uhs_phy_printfe("addr: 0x%x\r\n", STRESS_TEST_BASE);
                uhs_phy_printfe("data_w data0: 0x%x\r\n", data0);
                uhs_phy_printfe("addr_r data0: 0x%x\r\n", dataTmp);
    ...
    for (RA = 0x0; RA <= RowAddr; RA++){
        ...
        for (CA = CA_init; CA <= CA_init + 0x7; CA = CA + 2){
            ...
            if(dataTmp != data1){
                uhs_phy_printfe("addr: 0x%x\r\n", STRESS_TEST_BASE);
                uhs_phy_printfe("data_w data1: 0x%x\r\n", data1);
                uhs_phy_printfe("addr_r data1: 0x%x\r\n", dataTmp);
                return uhs_err_handler(UHS_DIAGONAL_TEST_ERR);
            }
        }
    }
...
uint8_t uhs_all_addr_test(void)
{
    ...
    for (RA = 0x0; RA <= RowAddr; RA++){
        ...
        for(CA = 0x0; CA <= 0x3ff; CA = CA + 2){
            ...
            if(dataTmp != data0){
                uhs_phy_printfe("addr: 0x%x\r\n", STRESS_TEST_BASE);
                uhs_phy_printfe("data_w data0 first: 0x%x\r\n", data0);
                uhs_phy_printfe("addr_r data0 first: 0x%x\r\n", dataTmp);
    ...
    for (RA = RowAddr; RA >= 0x0; RA--){
        ...
        for(CA = 0x3ff - 1; CA >= 0x0; CA = CA - 2){
            ...
            if(dataTmp != data0){
                uhs_phy_printfe("addr: 0x%x\r\n", STRESS_TEST_BASE);
                uhs_phy_printfe("data_w data0 second: 0x%x\r\n", data0);
                uhs_phy_printfe("addr_r data0 second: 0x%x\r\n", dataTmp);
                return uhs_err_handler(UHS_ALL_ADDR_TEST_ERR);
            }
        }
    }
...
void dump_uhs_phy_cal_res(void)
{
    ...
    uhs_phy_printf("uhs_phy_cal_res->crc_res = %x\r\n",uhs_phy_cal_res->crc_res);
    #else
    // dump uhs_phy_cal_res
    uhs_phy_printf("uhs_phy_cal_res->cal_mode = %d\r\n",uhs_phy_cal_res->cal_mode);
    uhs_phy_printf("uhs_phy_cal_res->err_type = %x\r\n",uhs_phy_cal_res->err_type);
    uhs_phy_printf("uhs_phy_cal_res->err_sub_type = %d\r\n",uhs_phy_cal_res->err_sub_type);
    uhs_phy_printf("uhs_phy_cal_res->datarate = %d\r\n",uhs_phy_cal_res->datarate);
    uhs_phy_printf("uhs_phy_cal_res->rl = %d\r\n",uhs_phy_cal_res->rl);
    uhs_phy_printf("uhs_phy_cal_res->rdqs = %d\r\n",uhs_phy_cal_res->rdqs);
    ...
    uhs_phy_printf("uhs_phy_cal_res->wwindow_begin = %d\r\n",uhs_phy_cal_res->wwindow_begin);
    uhs_phy_printf("uhs_phy_cal_res->wwindow_end = %d\r\n",uhs_phy_cal_res->wwindow_end);
    uhs_phy_printf("uhs_phy_cal_res->cal_done = %x\r\n",uhs_phy_cal_res->cal_done);
    uhs_phy_printf("uhs_phy_cal_res->crc_res = %x\r\n",uhs_phy_cal_res->crc_res);
    #endif
}
...
extern uint32_t BFLB_Soft_CRC32(void *dataIn, uint32_t len);
uint8_t uhs_phy_init_core(PSRAM_UHS_Cfg_Type *cfg)
{
    cal_done_flag = 0;
    ...
    if (cfg->psramMemSize == PSRAM_MEM_SIZE_32MB){
        ...
        // uhs_reg_r(0,1);
        // tmpVal = BL_RD_REG(PSRAM_UHS_BASE,PSRAM_UHS_UHS_CMD)>>24;
        // uhs_phy_printf_debug("readout MA0 = 0x%x\r\n",tmpVal);
